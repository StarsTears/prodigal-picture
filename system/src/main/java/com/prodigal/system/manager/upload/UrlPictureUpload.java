...
public class UrlPictureUpload extends PictureUploadTemplate{
        ...
        try {
            ...
            if (StrUtil.isNotBlank(contentLengthStr)){
                try {
                    ...
                    ThrowUtils.throwIf(contentLength > 10 * ONE_M, BizStatus.PARAMS_ERROR, "文件大小不能超过10M");
    ...
    protected void processFile(Object inputSource, File file) throws IOException {
        ...
        if (type == null || !type.matches("jpg|jpeg|png|webp")) {
            throw new BusinessException(BizStatus.PARAMS_ERROR, "下载的文件不是有效图片");
        }
        // 校验文件大小
        final long TEN_M = 10 * 1024 * 1024L;
        if (file.length() > TEN_M) {
            throw new BusinessException(BizStatus.PARAMS_ERROR, "文件大小不能超过10M");
        }
    }
