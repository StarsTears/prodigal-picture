...
public class DictServiceImpl extends ServiceImpl<DictMapper, Dict> implements DictService {
    ...
    public void add(DictDTO dto) {
        ...
        if (!save(dict)) {
            throw new BusinessException(BizStatus.OPERATION_ERROR, "添加失败");
        }
    }
    ...
    public void update(DictDTO dto) {
        if (dto.getId() == null) {
            throw new BusinessException(BizStatus.PARAMS_ERROR, "id不能为空");
        }
        Dict dict = new Dict();
        BeanUtils.copyProperties(dto, dict);
        if (!updateById(dict)) {
            throw new BusinessException(BizStatus.OPERATION_ERROR, "更新失败");
        }
    }
